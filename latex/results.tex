...
\caption{Multi-class classification summary: macro-averaged F1 (seed~42).
  ...
  tree baselines at \texttt{max\_depth=5}. DT achieves perfect separation via
  a depth-2 \texttt{DIntPkt} threshold chain confirmed by permutation importance
  (95.8\% single-feature dominance); RF near-perfect score ($0.9996$) reflects
  four bootstrap-variance misclassifications.
...
CIC-IoT2023 produces a 2$\times$ gap ($0.45$ vs.\ $0.89$), reflecting eight attack families with moderate inter-class overlap. The LLM generates correct per-class rules but cannot fully resolve the finer boundaries that gradient-based optimisation finds across the full training distribution.
...
The withheld class's feature-space distinctiveness is the primary determinant of ZDR, and the results confirm this pattern across all five scenarios. WUSTL-IIoT (Reconn) and TON\_IoT (ransomware) achieve near-perfect ZDR ($\geq 0.999$) with only modest reductions in Known-F1. For WUSTL-IIoT, this outcome follows directly from the feature-level evidence in Section~\ref{subsec:multiclass_results}. \texttt{DIntPkt} carries 95.8\% of permutation importance and provides a clean separation between Reconn and all other traffic classes. The rules trained on known classes therefore isolate Reconn flows with near-certainty even without observing them. For TON\_IoT, ransomware generates a sufficiently distinctive traffic signature that the trained rules transfer to unseen samples without substantially degrading Known-F1.
...
\begin{figure}[ht]
  ...
  \caption{Held-out test macro-averaged F1 at measured checkpoints per dataset:
    ...
    evaluated on the held-out test set. Dataset labels correspond to
    Table~\ref{tab:policy_refinement}.}
...
across all five datasets (4.5$\times$--80$\times$). Against Decision Tree,
policy inference is faster on CIC-IoT2023 and WUSTL-IIoT but slower on
TON\_IoT, Bot-IoT, and UNSW-NB15. The speed advantage over RF reflects
the absence of tree traversal. The mixed result against DT reflects that
shallow trees on low-dimensional feature sets can undercut the overhead of
iterating over a list of threshold conditions.
...
The distinct profile of each dataset determines whether precision or recall is the binding constraint on rule quality. CIC-IoT2023, WUSTL-IIoT, and Bot-IoT achieve high precision and recall ($\geq 0.96$, Table~\ref{tab:attack_precision_current}), confirming that well-separated class signatures allow the generated rules to separate attack from normal traffic with both high coverage and a low false-positive rate. UNSW-NB15 occupies an intermediate position (F1~$= 0.727$), where partial feature overlap depresses recall to $0.66$ while precision remains at $0.81$. TON\_IoT demonstrates the recall-collapse failure mode most sharply, where precision holds at $0.75$ but recall falls to $0.24$. This is the expected outcome for a dataset with heavy intra-class overlap across ten attack families, where threshold rules fire on a narrow subset of true attack traffic and miss three-quarters of genuine attack flows, consistent with the multi-class analysis in Section~\ref{subsec:multiclass_results}.
...
The LLM policy generator consistently identifies discriminative thresholds along feature axes that gradient-based importance ranking deprioritises, yielding a detection surface that is complementary to rather than redundant with either baseline. Feature overlap between the LLM rule sets and the RF and DT top 5 is limited across all five datasets (Table~\ref{tab:feature_use_current}), reaching a maximum of two shared features on Bot-IoT and falling to zero on UNSW NB15. The LLM-selected features concentrate on flow-level timing, connection-state, and address-based descriptors rather than the flag-count and statistical variance features that dominate the tree-based rankings. This divergence is a predictable consequence of how the LLM reasons from representative samples rather than optimising a gradient criterion over the full training distribution.
...
Fig.~\ref{fig:threshold_sensitivity_all} plots ZDR and FPR against the anomaly detection threshold $\tau$ for four zero-day scenarios. The dashed vertical line marks the operating point maximising ZDR\,$-$\,FPR.
...
CIC-IoT2023 and WUSTL-IIoT, with results validated over three random seeds. ESR curves are shown in Fig.~\ref{fig:robustness_curves}.
